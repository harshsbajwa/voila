...
export const HUD: React.FC = () => {
  ...
  if (isHidden) {
    return (
      <button 
        ...
        className="absolute top-5 left-5 bg-black/80 backdrop-blur-lg border border-white/10 rounded-xl p-3 z-10 text-gray-400 hover:text-white transition-colors"
        aria-label="Show Market Overview"
      >
        📊
      </button>
    )
  ...
  return (
    <div className={`hud ${isCollapsed ? 'collapsed' : ''}`}>
      <div className="hud-header">
        <h2>{displayTitle}</h2>
        <button 
          onClick={() => setIsHidden(true)}
          className="hud-toggle text-gray-400 hover:text-white transition-colors touch-optimized"
          aria-label="Close"
        >
          ✕
        </button>
